...
class Calibration:
    ...
    def do_cali(self, cap):
        object_point = np.zeros((9 * 6, 3), np.float32)
        object_point[:, :2] = np.mgrid[0:9, 0:6].T.reshape(-1, 2)
        key = -1

        while len(self.object_points) < 15:
            _, frame = cap.read()
            cv2.imshow("frame", frame)
            grayed_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            retCorner, corners = cv2.findChessboardCorners(
                grayed_frame, (self.NX, self.NY), None
            )
            if retCorner and key != -1:
                corner2 = cv2.cornerSubPix(
                    grayed_frame, corners, self.WIN_SIZE, self.ZERO_ZONE, self.CRITERIA
                )
                cv2.drawChessboardCorners(
                    grayed_frame, (self.NX, self.NY), corners, True
                )
        ...
        ret, camera_matrix, distortion_coeffs, rvecs, tvecs = cv2.calibrateCamera(
            self.object_points, self.img_points, (self.NX, self.NY), None, None
        )
...
if __name__ == "__main__":
    ...
    cali = Calibration()
    cameraMatrix, distCoeffs = cali.do_cali(cap)
