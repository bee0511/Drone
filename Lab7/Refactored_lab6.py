...
def tracking_aruco(frame, marker_id, z_distance):
    ...
    for i in range(rvec.shape[0]):
        ...
        return (modified_frame, x_update, y_update, z_update, yaw_update)
...
def main():
    ...
    while True:
        ...
        key = cv2.waitKey(33)
        packed_data = tracking_aruco(frame, 0, 120)
        if not packed_data:
            if key != -1:
                keyboard(drone, key)
            else:
        ...
        tagged_frame, x_update, y_update, z_update, yaw_update = packed_data
        if key != -1:
            keyboard(drone, key)
